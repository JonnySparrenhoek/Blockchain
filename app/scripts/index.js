...
const App = {
  ...
  stopBidding: function () {
    ...
    return new Promise(function (resolve, reject) {
      InvoiceFactoring.deployed().then(function (instance) {
        return instance.stopBidding(id, accept, { from:exporterAccount, gas:gasLimit })
      }).then(function (value) {
  ...
  withdrawTokens: function(){
    ...
    fetch(endpoint).then(function(response){
      ...
      FiatConversion.deployed().then(function(instance){
        convertedToUSD = parseInt(convertedToUSD);
        return instance.transferToken(defaultAccount, { from: investorAccount, gas:gasLimit, value: web3.toWei(convertedToUSD, 'ether') });
      })
  ...
  showRates: function(){
    ...
    fetch(endpoint).then(function(response){
      ...
      //alert('view logs for response')
      document.getElementById("EXCshow").innerHTML = "Current exchange rates for our tokens";
      document.getElementById("USDshow").innerHTML = "US Dollar: " + 1 + " USD equals one token";
      document.getElementById("SGDshow").innerHTML = "Singapore Dollar: " + jsonResponse.rates.SGD / jsonResponse.rates.USD + " SGD equals one token";
      document.getElementById("EURshow").innerHTML = "Euro: " + 1/jsonResponse.rates.USD + " EUR equals one token";
    })
  },  
  ...
  placeBid: function() {
    ...
    InvoiceFactoring.deployed().then(
      function(instance){
        ...
        return instance.placeBid(id,{from:investorAccount, gas:gasLimit, value: web3.toWei(amt, 'ether') })
      }
    ).then(
      function(value){
        console.log("log: ")
